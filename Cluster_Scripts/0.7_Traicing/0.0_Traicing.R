...
# PATHS
# ...............................................................................
path_mt20_EC <- "/ijc/LABS/GRAUPERA/RAW/SPETRISSANS/ANNA_MARIA/3.MERGE/0.4_SubsetEndothelial/mt15/9.RemoveCluster/2.Integration/SubsetEndothelial_Harmony.rds"
#path_mt15_EC <- "/ijc/LABS/GRAUPERA/RAW/SPETRISSANS/ANNA_MARIA/1.Unenriched_samples/0.0_Previously_processed_by_ane/mt15/Data_Annotated_mt15_EC.rds"
path_mt15_EC <- "/ijc/LABS/GRAUPERA/RAW/SPETRISSANS/ANNA_MARIA/3.MERGE/0.5_Annotation_EC/mt20/5.Annotation/Seurat_annotated.rds"

output_dir <- "/ijc/LABS/GRAUPERA/RAW/SPETRISSANS/ANNA_MARIA/3.MERGE/0.7_Traicing/mt15/subset2/traicing_annotation_mt20"
...
layer_levels <- levels(seurat_mt15_EC$Annotation_Layer2)
...
p1 <- DimPlot(seurat_mt15_EC, group.by = "Annotation_Layer2", label = FALSE, pt.size = 1.5, cols = palette_fixed) & NoAxes()
ggsave(plot = p1, filename = file.path(output_dir, "mt15_Layers_EC.png"), width = 10, height =8)


# 1. Extraer anotación original
tracing_annotation <- seurat_mt15_EC@meta.data[, "Annotation_Layer2", drop = FALSE]

# Crear versiones limpias de los nombres (sin sufijos)
colnames_mt20_clean <- sub("_\\d+$", "", colnames(seurat_mt20_EC))
rownames_mt15_clean <- sub("_\\d+$", "", rownames(tracing_annotation))

# Crear tabla de mapping para intersección
names(colnames_mt20_clean) <- colnames(seurat_mt20_EC)
...
seurat_mt20_EC$Layer_EC_1_traced <- NA
seurat_mt20_EC$Layer_EC_1_traced[cells_mt20_matched] <- as.character(matched_annotation$Annotation_Layer2)

# Convertir a factor
seurat_mt20_EC$Layer_EC_1_traced <- factor(
  seurat_mt20_EC$Layer_EC_1_traced,
  levels = levels(tracing_annotation$Annotation_Layer2)
)


# 3. Visualizar
p2 <- DimPlot(seurat_mt20_EC, group.by = "Layer_EC_1_traced", label = FALSE, pt.size = 1.5, cols = palette_fixed) & NoAxes()
...
categorias <- levels(seurat_mt20_EC$Layer_EC_1_traced)
...
for (cat in categorias) {
  ...
  # Crear el plot
  p <- DimPlot(seurat_mt20_EC, group.by = "Layer_EC_1_traced", cols = cols_destacadas, pt.size = 1.5) & NoAxes() & NoLegend()
