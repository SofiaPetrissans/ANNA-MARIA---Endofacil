...
SimpleAnnotator <- function(seu, resolution, plot_dir) {
  ...
  gene_sets <- list(
    ...
    LVEC = c("Cdh5", "Pecam1", "Cd31", "Vwf", "Flt1", "Tek", "Nos3"),
...
ClusterMarkers <- function(seu, resolution, marker_dir) {
  seu <- SetIdent(seu, value = resolution)
  clusters <- levels(seu)
...
ClusterMarkers <- function(seu, resolution, marker_dir, plot_dir) {
  ...
  # Concatenar total
  total_markers <- bind_rows(all_markers)
  write.xlsx(total_markers, file.path(marker_dir, paste0("Markers_res", RESOLUTION, "_Total.xlsx")))

  # ⚡ NUEVO: DotPlot de top 4 genes por clúster
  top_markers <- total_markers %>%
    group_by(cluster) %>%
    top_n(4, avg_log2FC) %>%
...
# Dim Plot para Doublet Finder y Scrublet ................................................

# Buscar el nombre de la columna de clasificación de DF
df_col <- colnames(seurat_object@meta.data)[grepl("^DF.classifications", colnames(seurat_object@meta.data))][1]

# UMAP coloreado por clasificación DoubletFinder

p_df <- DimPlot(seurat_object group.by = df_col, pt.size = 1, raster = FALSE) & NoAxes()
ggsave(filename = file.path(plot_dir, "DoubletFinder_Classification.png"), plot = p_df, width = 7, height = 7)


p <- DimPlot(seurat_object, group.by = "Scrublet_Classification", pt.size = 1, raster = FALSE) & NoAxes()
ggsave(filename = file.path(plot_dir, "Scrublet_Classification.png"), plot = p, width = 7, height = 7)
